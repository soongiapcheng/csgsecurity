...
# Extract ONLY the bot token pattern (e.g. 123456789:ABCdefGHI...)
# Strips out any accidental Markdown or [https://...](https://...) wrapping the secret
token_match = re.search(r'\d+:[A-Za-z0-9_-]+', RAW_TOKEN)
token = token_match.group(0) if token_match else RAW_TOKEN

# Extract ONLY digits for chat ID (handles negative numbers for group/channel IDs)
chat_match = re.search(r'-?\d+', RAW_CHAT_ID)
chat_id = chat_match.group(0) if chat_match else RAW_CHAT_ID
...
2. Use ONLY simple, fully-closed raw HTML tags: <b>bold</b> for headings/emphasis, <i>italic</i> for extra detail.
...
try:
    ...
    # Fallback to plain text if HTML tags generated by AI cause a parse error
    if not response.ok:
        print(f"HTML send failed ({response.status_code}): {response.text}. Retrying plain text...")
        clean_text = re.sub(r'<[^>]+>', '', tg_response.text[:3000])
        plain_msg = f"🔒 Daily Cyber Briefing\n\n{clean_text}\n\n🔗 View Full Report: {page_url}"
        response = requests.post(
            telegram_url,
            json={
                "chat_id": chat_id,
